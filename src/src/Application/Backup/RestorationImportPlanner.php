...
{
    private const ORDER = ['accounts', 'categories', 'recurrences', 'commitments', 'entries', 'theme'];
    ...
    public function plan(StagedBackup $backup, int $effectiveUserId): RestorationImportPlan
    {
        if ($effectiveUserId <= 0
            || ($backup->metadata['owner'] ?? null) !== $effectiveUserId) {
            throw new \InvalidArgumentException('Backup incompatível com o usuário efetivo.');
        }
        ...
        foreach (array_slice(self::ORDER, 0, -1) as $section) {
            ...
            foreach ($backup->{$section} as $index => $record) {
                ...
                if (isset($maps[$section][$identity])) {
                    throw new \InvalidArgumentException('Identificador duplicado em ' . $section . '.');
                }
        ...
        foreach (array_slice(self::ORDER, 0, -1) as $section) {
            ...
            foreach ($backup->{$section} as $index => $record) {
                $identity = isset($record['id']) && $record['id'] !== null
                    ? (string) $record['id']
                    : 'index-' . $index;
                ...
                foreach (self::REFERENCES[$section] ?? [] as $field => $target) {
                    ...
                    $lookup = is_int($record[$field])
                        ? (string) $record[$field]
                        : $record[$field];

                    if (!is_string($lookup) || !isset($maps[$target][$lookup])) {
                        throw new \InvalidArgumentException('Referência não resolvida em ' . $section . '.');
                    }
        ...
        return new RestorationImportPlan(
            $effectiveUserId,
            self::ORDER,
            $records,
            $maps,
    ...
    private function assertUser(array $record, int $userId): void
    {
        foreach (['userId', 'user_id'] as $field) {
            if (array_key_exists($field, $record) && $record[$field] !== $userId) {
