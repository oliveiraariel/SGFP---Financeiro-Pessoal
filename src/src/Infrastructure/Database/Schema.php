...
CREATE TABLE IF NOT EXISTS {$recurrence} (
    ...
    criada_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_recorrencia_usuario FOREIGN KEY (fk_id_usuario) REFERENCES {$usersTable}(ID),
    CONSTRAINT ck_recorrencia_inicio_mes CHECK (DAY(inicio_mes) = 1),
    CONSTRAINT ck_recorrencia_quantidade CHECK (quantidade_meses IS NULL OR quantidade_meses > 0),
...
CREATE TABLE IF NOT EXISTS {$commitment} (
    ...
    CONSTRAINT fk_compromisso_usuario FOREIGN KEY (fk_id_usuario) REFERENCES {$usersTable}(ID),
    CONSTRAINT fk_compromisso_categoria FOREIGN KEY (fk_id_categoria, fk_id_usuario)
        REFERENCES {$category}(id_categoria, fk_id_usuario),
    CONSTRAINT fk_compromisso_recorrencia FOREIGN KEY (fk_id_recorrencia, fk_id_usuario)
...
CREATE TABLE IF NOT EXISTS {$entry} (
    ...
    CONSTRAINT fk_lancamento_usuario FOREIGN KEY (fk_id_usuario) REFERENCES {$usersTable}(ID),
    ...
    CONSTRAINT ck_lancamento_origem CHECK (origem IN ('COMPROMISSO', 'SALDO_INICIAL')),
    CONSTRAINT ck_lancamento_origem_compromisso CHECK (
        (origem='COMPROMISSO' AND fk_id_compromisso IS NOT NULL AND valor >= 0)
        OR (origem='SALDO_INICIAL' AND fk_id_compromisso IS NULL)
    ),
    CONSTRAINT ck_lancamento_tipo_efeito CHECK (tipo_efeito IN ('ENTRADA', 'SAIDA')),
    CONSTRAINT ck_lancamento_saldo_inicial CHECK (origem <> 'SALDO_INICIAL' OR tipo_efeito='ENTRADA'),
    CONSTRAINT ck_lancamento_estado CHECK (estado IN ('ATIVO', 'DESFEITO')),
    CONSTRAINT ck_lancamento_desfazimento CHECK (
        (estado='ATIVO' AND desfeito_em IS NULL)
        OR (estado='DESFEITO' AND desfeito_em IS NOT NULL)
    ),
    CONSTRAINT uq_lancamento_compromisso_ativo UNIQUE (id_compromisso_ativo),
